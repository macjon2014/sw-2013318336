	...
	private void InfoChange(){
		...
		if (!userName.getText().toString().equals("")) {
			if (cursor != null) {
				...
				Toast.makeText(getApplicationContext(), "정보수정완료",
			...
			Toast.makeText(getApplicationContext(), "입력해주세요",//用户名
					Toast.LENGTH_SHORT).show();
	...
	private void PasswordChange(){
		...
		if (str_bundlePass.equals(str_old)) {
			if (str_news.equals(str_sure_news)) {
				...
				Toast.makeText(getApplicationContext(), "비밀 번호가 일치하지 않습니다.",
						Toast.LENGTH_SHORT).show();
				old.setText("");
				news.setText("");
				sure_news.setText("");
			...
			Toast.makeText(getApplicationContext(), "원래 비밀 번호가 틀렸습니다.",
					Toast.LENGTH_SHORT).show();
			old.setText("");
			news.setText("");
			sure_news.setText("");
