...
public class GoodsInformation extends Activity {
	...
	private void AddGoodsData() {
		if(info==null){
			Toast.makeText(getApplicationContext(), "판매자 없습니다，상품발표 할수 없습니다.", Toast.LENGTH_SHORT)
			.show();
			return;
		}
		if(!name.getText().toString().equals("")&&!price.getText().toString().equals("")&&!describe.getText().toString().equals("")){
			...
			price.setText("");
			describe.setText("");
			Toast.makeText(getApplicationContext(), "추가완료", Toast.LENGTH_SHORT)
					.show();
			cursor.close();
			db.close();
			myHelper.close();
		}else {
			Toast.makeText(getApplicationContext(), "정보입력하세요", Toast.LENGTH_SHORT);
		}
