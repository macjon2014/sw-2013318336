...
public class ProductMessage extends Activity{
	...
	private void Buy(){
		if(type.equals("visitor")){
			Toast.makeText(getApplicationContext(), "방문객은 상품 구매 불가", 1).show();
			return;
		}else{
			db = helper.getWritableDatabase();
			...
			if(cursor!=null){
				...
				Toast.makeText(getApplicationContext(), "구매완료,카트에 담았습니다.", Toast.LENGTH_SHORT).show();
			}
			cursor.close();
			db.close();
			helper.close();
