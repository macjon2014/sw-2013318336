		...
		if (password1.getText().toString()
			...
			if (loginType == "판매자") {
					...
					for (int i = 0; i < cursor.getCount(); i++) {
						...
						if (account
							...
							Toast.makeText(getApplicationContext(),
									"이미 가입된 아이디 입니다.", Toast.LENGTH_SHORT)
									.show();
							account.setText("");
							password1.setText("");
							password2.setText("");
				...
				if (temp == 0) {
					...
					Toast.makeText(getApplicationContext(),
							"가입완료,다시 로그인 해주세요", Toast.LENGTH_SHORT).show();
					Login.this.finish();
				}
				cursor.close();
			} else if (loginType == "구매자") {
				int temp1 = 0;
				...
				if (cursor != null) {
					for (int i = 0; i < cursor.getCount(); i++) {
						...
						if (account
							...
							Toast.makeText(getApplicationContext(),
									"이미 가입되 아이디 입니다.", Toast.LENGTH_SHORT)
									.show();
							account.setText("");
							password1.setText("");
							password2.setText("");
